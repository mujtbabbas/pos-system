...
class Config extends Secure_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->db = Database::connect();
         $this->employees = model(Employee::class);
        helper('security');
        if (check_encryption()) {
    ...
    private function _licenses(): array    // TODO: remove hungarian notation.  Super long function.  Perhaps we need to refactor out functions?
    {
        ...
        $license[$i]['title'] = 'SaleMate POS ' . config('App')->application_version;

        if (file_exists('license/LICENSE')) {
    $license_text = file_get_contents('license/LICENSE', false, null, 0, 3000);
    // Change the name just for display without touching the file itself
    $license_text = str_replace('Open Source Point of Sale', 'SaleMate POS', $license_text);
    $license[$i]['text'] = $license_text;
} else {

            $license[$i]['text'] = 'LICENSE file must be in SaleMate POS license directory. You are not allowed to use SaleMate POS application until the distribution copy of LICENSE file is present.';
        }
...
public function getIndex(): void
{
    ...
    // -- ADD THIS CODE BLOCK TO YOUR getIndex() METHOD --

    // Fetch all employees to populate the waiter dropdown
    ...
    // Get the currently selected waiters from the config
    $waiter_names_config = $this->config['waiter_names'] ?? '';
    $data['waiter_names'] = explode(',', $waiter_names_config);
    ...
    public function postCheckNumberLocale(): void
{
    ...
    // Always get currency symbol and code from the form first, then fall back to formatter defaults
    $currency_symbol = $this->request->getPost('currency_symbol');
    $currency_code = $this->request->getPost('currency_code');
    
    // Only use formatter defaults if the form fields are empty
    ...
    // Update save_number_locale only if it's different from current
...
/**
 * Loads the table and waiter configuration view.
 * @return string
 */
public function getDinnerTables(): string
{
    ...
    $data = [];
    $data['controller_name'] = strtolower($this->getUri()->getSegment(1));
    $data['config'] = config(OSPOS::class)->settings;

    // Fetch the dinner table data
    $dinner_tables_query = $this->dinner_table->get_all();
    $data['dinner_tables'] = [];
    foreach ($dinner_tables_query->getResultArray() as $row) {
        $data['dinner_tables'][$row['dinner_table_id']] = $row['name'];
    }

    // Correctly fetch and format the employee list
    $employees_query = $this->employees->get_all();
    $employees_list = [];
    foreach ($employees_query->getResultArray() as $row) {
        $employees_list[$row['person_id']] = $row['first_name'] . ' ' . $row['last_name'];
    ...
    // Correctly get the current waiter configuration from the loaded settings
    // The value should be a comma-separated string, so we use explode.
    $waiter_names_config = $data['config']['waiter_names'] ?? '';
    $data['waiter_names'] = explode(',', $waiter_names_config);

    return view('configs/table_config', $data);
}
// ... other methods ...
     ...
     */
    /**
     * Saves all dinner tables and selected waiters. Used in app/Views/configs/table_config.php
     *
     * @throws ReflectionException
     * @return void
     * @noinspection PhpUnused
...
public function postSaveTables(): void
{
    ...
    if ($dinner_table_enable) {
        $not_to_delete = [];
        foreach ($this->request->getPost() as $key => $value) {
            if (str_contains($key, 'dinner_table') && $key != 'dinner_table_enable') {
    ...
    // NEW: Waiter Logic
    $waiter_enable = $this->request->getPost('waiter_enable') != null;
    $waiter_names = $this->request->getPost('waiter_names') ?? [];

    $config_data = [
        'waiter_enable' => $waiter_enable,
        'waiter_names' => serialize($waiter_enable ? $waiter_names : [])
    ];
    $this->appconfig->batch_save($config_data);
