...
class Sale_lib
{
    ...
    {
        // Debugging line for browser console
        echo '<script>console.log("DEBUG: Sale_lib->get_dinner_table() called. Session value is:", ' . json_encode($this->session->get('dinner_table')) . ');</script>';
        ...
        $table_id = $this->session->get('dinner_table');
        
        // Debugging line to display the final returned value
        echo '<script>console.log("DEBUG: Sale_lib->get_dinner_table() returning ID:", ' . json_encode($table_id) . ');</script>';
   ...
   public function set_dinner_table(?int $dinner_table): void
    {
        // Debugging line for browser console
        echo '<script>console.log("DEBUG: Sale_lib->set_dinner_table() called with ID:", ' . json_encode($dinner_table) . ');</script>';

        $this->session->set('dinner_table', $dinner_table);

        // Debugging line to confirm the session variable was set
        echo '<script>console.log("DEBUG: Sale_lib->set_dinner_table() session value is now:", ' . json_encode($this->session->get('dinner_table')) . ');</script>';
    }
    /**
     * @return void
     */
    ...
    public function delete_item(int $line): void
{
    ...
    // Check if the line exists in the cart
    if (!isset($items[$line])) {
        // If the line doesn't exist, do nothing
        return;
    }
    ...
    // In app/Libraries/Sales_lib.php

// New function to set the waiter ID
public function set_waiter(int $waiter_id)
{
    // Log the action for debugging
    log_message('info', "DEBUG: Sale_lib->set_waiter() called. Setting waiter ID to: " . $waiter_id);

    $this->session->set('waiter_id', $waiter_id);
}

// New function to get the waiter ID
public function get_waiter(): int|null
{
    $waiter_id = $this->session->get('waiter_id');

    // Log the action for debugging
    log_message('info', "DEBUG: Sale_lib->get_waiter() called. Session value is: " . ($waiter_id ?? 'null'));

    return $waiter_id;
}
public function set_current_order_id(?int $order_id): void {
    session()->set('current_order_id', $order_id);
}


public function get_current_order_id(): ?int {
    return session()->get('current_order_id');
}
public function sync_order_items($order_id)
{
    $cart_items = $this->get_cart(); // current cart items
    $order_item_model = model(\App\Models\OrderItem::class);

    // Fetch existing items for THIS order only
    $db_items = $order_item_model->get_order_items($order_id);
    $db_map = [];
    foreach ($db_items as $db_item) {
        $db_map[$db_item->item_id] = $db_item; // key by item_id
    }

    // Insert or update items for THIS order only
    foreach ($cart_items as $cart_item) {
        ...
        $item_data = [
            ...
            'quantity' => $cart_item['quantity'], // cart quantity
            'unit_price' => $cart_item['price'],
            'discount_percent' => $cart_item['discount'],
        ];

        if (isset($db_map[$item_id])) {
            // Update only this order_id + item_id
            $builder = $order_item_model->builder();
            $builder->where('order_id', $order_id)
                    ->where('item_id', $item_id)
                    ->update($item_data);
        } else {
            // Insert new item for this order
            $order_item_model->save_item($item_data);
        }
    }

    // Delete items removed from cart for THIS order only
    foreach ($db_map as $item_id => $db_item) {
        $found = false;
        foreach ($cart_items as $cart_item) {
        ...
        if (!$found) {
            $order_item_model->delete_item_from_order($order_id, $item_id);
        }
    }
}
