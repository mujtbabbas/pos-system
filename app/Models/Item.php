...
/**
     * Get all categories (distinct) from items table
     * @return array
     */
  ...
  public function get_items_by_category($category)
{
    return $this->db->table('items')
        ->select('item_id, item_number, name, category, cost_price, unit_price, quantity, tax_category_id')
...
public function get_all_items(): array
{
    return $this->db->table('items i')
        ->select('
            ...
            i.cost_price,
            i.unit_price,
            COALESCE(SUM(iq.quantity), 0) AS quantity,
            i.tax_category_id
        ')
        ->join('item_quantities iq', 'iq.item_id = i.item_id', 'left')
        ->where('i.deleted', 0)
        ->groupBy('i.item_id, i.item_number, i.name, i.category, i.cost_price, i.unit_price, i.tax_category_id')
        ->orderBy('i.name')
        ->get()
        ->getResultArray();
}
